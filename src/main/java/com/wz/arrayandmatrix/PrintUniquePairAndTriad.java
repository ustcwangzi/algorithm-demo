 ...
 *        当三元组第一个数值为-8时，寻找-8后面的子数组中所有相加为18的不重复二元组；
 *        当三元组第一个数值为-4时，寻找-4后面的子数组中所有相加为14的不重复二元组；
 *        当三元组第一个数值为-3时，寻找-3后面的子数组中所有相加为13的不重复二元组。
 *        以此类推。
 *        在选择三元组第一个值时，检查是否与它的前一个值相等，若相等则跳过，保证结果不重复。
 * </p>
 * <p>
