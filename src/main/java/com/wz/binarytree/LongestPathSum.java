 ...
 * <p>累加和为指定值的最长路径长度</p>
 * <p>
 *     二叉树节点值类型为整形，求累加和为target的最长路径长度，不用考虑相加溢出的情况
 *     1、头节点head、指定值target均已知，变量maxLen用以记录累加和为target的最长路径长度
 ...
 *        若sumMap中已包含curSum则不更新，否则将(curSum,level)放入sumMap，遇到累加和为target时，更新maxLen，然后遍历cur的左右子树
...
public class LongestPathSum {
    ...
    private static int preOrder(Node head, int target, int preSum, int level, int maxLen, Map<Integer, Integer> sumMap) {
        if (head == null) {
            return maxLen;
        }
        ...
        if (sumMap.containsKey(curSum - target)) {
            maxLen = Math.max(level - sumMap.get(curSum - target), maxLen);
        }
    ...
    public static void main(String[] args) {
        ...
        for (int i = -9; i < 6; i++){
            System.out.print(getMaxLength(head, i) + " ");
        }
    }
}
