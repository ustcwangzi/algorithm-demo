 ...
 *     解决方案二：
 *        使用"四边形不等式"优化动态规划。
 *        假设计算dp[i-1][j]时，在最好的划分方案中，第i-1个画匠负责arr[a…j]的画作。
 *        在计算dp[i][j+1]时，在最好的划分方案中，第i个画匠负责arr[b…j]的画作。
 *        那么在计算dp[i][j]时，假设最好的划分方式是让第i个画匠负责arr[k…j]，那么k的范围一定是[a, b]，这样就省去了很多无效的枚举过程。
 *     解决方案三：
