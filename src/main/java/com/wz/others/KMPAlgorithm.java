 ...
 *            -------
 *     在str[6]处失配，那么主字符串和模式字符串的前6位就是相同的，即 str[0...5]==match[0...5]，
 *     又因为模式字符串的前6位中，前4位前缀和后4位后缀是相同的(pmt[5]==4)，得到 match[0...3]==match[2...5]，
 *     所以我们推知 str[2...5]==match[0...3]，就是图中的划线部分，那这部分就不用再比较了，从match[4]开始向后匹配。
 *
 ...
 *     匹配成功时，有next[j]=k；失配时，令k=next[k]继续匹配(因为next[k]之前的字符都已经是匹配过的)。
