 ...
 * <p>正数数组的最小不可组成和</p>
 * <p>
 *     给定一个正整数数组array，其中所有的值都为整数，以下是最小不可组成和的概念：
 ...
 *        如果array[0...i]成的子集可以累加出k，那么array[0...i+1]的子集中必然可以累加出k+array[i]。
 *     一定包含1的情况：
 *        1、将array进行排序，排序后必然有array[0]==1；
 *        2、从左往右计算每个位置i的range。range表示当计算到位置array[i]时，[1,range]上所有数都能被array[0...i-1]的子集累加出来。
 *           初始时range=0；
 *        3、如果array[i]>range+1，因为array是有序的，后续不会再出现range+1，即array[0...i]上，range+1这个数一定不能累加出来，
 *           返回range+1即可。
 *           如果array[i]<=range+1，说明[1,range+array[i]]区间上所有的正数都可以被array[0...i]上的某一个子集累加出来，
 *           所以令range+=array[i]，然后继续计算下一个位置。
