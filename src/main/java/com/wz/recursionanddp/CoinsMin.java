 ...
 *     方案一：
 *        生成行数为N、列数为aim+1的动态规划表dp，dp[i][j]代表在任意使用array[0...i]货币的情况下，组成j所需的最小货币张数，计算：
 *        1、dp[0...N][0]的值，即矩阵第一列的值，表示aim为0时需要的最小张数，不需要任何货币，全设为0即可
 *        2、dp[0][0...aim]的值，即矩阵第一行的值，表示只能使用array[0]货币时组成j所需的最小张数。比如array[0]=2，
 *           那么能组成的aim为2,4,6,...所以令dp[0][2]=1，dp[0][4]=2，...其余设为整数的最大值，记为MAX
 *        3、剩余的位置依次从左到右，再从上到下计算。假设计算到位置dp[i][j]，dp[i][j]的值可能来自下面的情况：
 *           完全不使用当前货币array[i]下的最小张数，即dp[i-1][j]
 *           只使用1张当前货币array[i]下的最小张数，即dp[i-1][j-array[i]]+1
 *           只使用2张当前货币array[i]下的最小张数，即dp[i-1][j-2*array[i]]+2
 *            ... ...
 *           只使用k张当前货币array[i]下的最小张数，即dp[i-1][j-k*array[i]]+k
 *           所有情况下，最终取张数最小的：
 ...
 *           如果j-array[i]<0，即发生越界，说明array[i]太大，用一张都会超过j，令dp[i][j] = dp[i-1][j]即可
 *     方案二：
 ...
 *           令dp[0][2]=1，因为只能使用一张，其余设为整数的最大值，记为MAX
 *        3、剩余的位置依次从左到右，再从上到下计算。假设计算到位置dp[i][j]，dp[i][j]的值可能来自下面的情况：
 *           不使用当前货币array[i]下的最小张数，即dp[i-1][j]
 *           因为每个货币只能使用一张，考虑dp[i-1][j-array[i]]的值，这个值代表任意使用array[0...i-1]货币情况下，
 *           组成j-array[i]所需的最小张数，从钱数为j-array[i]到钱数j，只要加上这张array[i]即可，即dp[i-1][j-array[i]]+1
 *           所有情况下，最终取张数最小的：
...
public class CoinsMin {
    ...
    public static int uniMinCoinsOne(int[] array, int aim) {
        ...
        int[][] dp = new int[array.length][aim + 1];
        // 第一行只能使用一张array[0]，只有dp[0][array[0]]=1，其他均为MAX
        for (int j = 1; j <= aim; j++) {
            dp[0][j] = MAX;
        }
        if (array[0] <= aim) {
    ...
    public static int uniMinCoinsTwo(int[] array, int aim) {
        ...
        int[] dp = new int[aim + 1];
        // 初始化只能使用一张array[0]，只有dp[array[0]]=1，其他均为MAX
        for (int j = 1; j <= aim; j++) {
            dp[j] = MAX;
        }
        if (array[0] <= aim) {
