 ...
 * <p>单词查找树</p>
 * <p>
 *     在单词查找树中，值是从根节点到含有非空值的节点的路径所隐式表示的
 *     例如sea，数据结构即没有保存字符串sea，也没有保存字符s、e、a
 *     事实上，数据结构不会保存任何字符串或字符，而是保存了链接数组和值
 *     Node中保存键相关联的值并用数组保存所有指向其他Node对象的引用
 * </p>
 * <p>单词查找树的链表结构和键的插入或删除顺序无关，对任意给定的一组键，其单词查找树唯一</p>
 * <p>单词查找树中查找一个键或插入一个键时，访问数组的次数是最长键的长度加一</p>
 * <p>一棵单词查找树中的链接总数在 RN 到 RNW 直接，R为字母表大小，W为键的平均长度</p>
 *
 * @author wangzi
 */
