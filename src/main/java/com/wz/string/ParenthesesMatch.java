 ...
 *             所以如果i-dp[i-1]-1位置上的字符是'('，就能够与当前位置的str[i]再匹配出一对有效括号，如果"(()())"，假设遍历到最后一个')'，
 *             以倒数第二个字符结尾的最长有效括号字串是"()()"，找到这个子串之前的字符，即i-dp[i-1]-1位置的字符，发现是'('，
 *             可以与当前')'再匹配出一对有效括号，此时dp[i]=dp[i-1]+2；
 *             另外，还有一种情况，比如"()(())"，假设遍历到最后一个')'，通过上面的过程找到的最长有效括号子串为"(())"，但是前面还有"()"，
